...
export const DemoInterface = ({
  ...
  const formatTextWithBoldTitles = (text: string): string => {
    ...
    const formattedLines = lines.map(line => {
      ...
      if (line.match(/^(TYPES?|CATEGORIES|BENEFITS?|FEATURES?|MECHANISMS?|BACKING|COLLATERAL|RISKS?|ADVANTAGES?|DISADVANTAGES?|COMPARISON|OVERVIEW|SUMMARY|CONCLUSION|USECASES?|ASSOCIATED INSTITUTIONS?|NATIVE STABLECOIN|LAUNCH STATUS|KEY BACKERS|UNIQUE EDGE)[A-Za-z\s]*$/i)) {
        return `<strong>${line}</strong>`;
      }
